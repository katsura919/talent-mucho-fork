...
export function WelcomeInteractive({ C, mono, sans, serif, scale = 1, segments, timerSecs }: SlideProps & {
  segments: Segment[];
  timerSecs: number;
}) {
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '24px auto 0' }}>
      ...
      <div style={{
        background: C.text, color: onDark, borderRadius: 24, padding: '40px 36px',
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 24, flexWrap: 'wrap' }}>
          ...
          {/* Live cities feed */}
          <div style={{ minWidth: 280, padding: '18px 22px', borderRadius: 14, background: 'rgba(250,248,245,0.06)', border: `1px solid rgba(250,248,245,0.12)`, position: 'relative' }}>
            ...
            <div style={{ minHeight: sz(28), display: 'flex', alignItems: 'center' }}>
              <div key={cityIdx} style={{ ...sans, fontSize: sz(18), fontWeight: 600, color: onDark, letterSpacing: '-0.01em', animation: 'fadeInUp 0.4s ease' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 26 }}>
                ...
                {realPains.map((m: { firstName: string; painPoint: string; painCategory: string }, i: number) => {
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      <div style={{ width: sz(28), height: sz(28), flexShrink: 0, borderRadius: '50%', background: C.primary, color: C.text === '#2A2520' ? onDark : C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', ...mono, fontSize: sz(11), fontWeight: 800 }}>
        ...
        <div style={{ padding: '28px 30px', borderRadius: 18, background: C.surface, border: `1px solid ${C.border}` }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {segments.map(s => (
              <div key={s.id} style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: 12, alignItems: 'center', padding: '10px 12px', borderRadius: 10, background: s.id === 0 ? `${C.primary}15` : 'transparent', border: `1px solid ${s.id === 0 ? `${C.primary}40` : 'transparent'}` }}>
                <div style={{ width: sz(28), height: sz(28), flexShrink: 0, borderRadius: '50%', background: s.id === 0 ? C.primary : `${C.muted}15`, color: s.id === 0 ? (C.text === '#2A2520' ? onDark : C.bg) : C.muted, display: 'flex', alignItems: 'center', justifyContent: 'center', ...mono, fontSize: sz(11), fontWeight: 800 }}>{s.num}</div>
      ...
      {/* QR Code */}
      <div style={{ marginTop: 26, padding: '28px 30px', borderRadius: 18, background: C.surface, border: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: 28 }}>
        <div style={{ flexShrink: 0, width: sz(160), height: sz(160), borderRadius: 14, background: '#FFFFFF', padding: 8, border: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ ...mono, fontSize: sz(12), color: C.text, letterSpacing: '0.04em', padding: '8px 14px', borderRadius: 8, background: `${C.primary}10`, border: `1px solid ${C.border}`, display: 'inline-block' }}>
...
export function LocationCloud({ C, mono, sans, serif, scale = 1 }: SlideProps) {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const { data: rows } = await supabase.from('workbook_responses').select('response_text').eq('segment_num', '00');
      if (!cancelled && rows) {
        let freq: Record<string, number> = {};
        for (const r of rows) { const { updated } = mergeLocation(r.response_text, freq); freq = updated; }
        setLocations(freq);
      }
      const ch = supabase.channel('loc-cloud-00').on(
        'postgres_changes',
        ...
        (payload: { new: { response_text: string } }) => {
          ...
          setLocations(prev => {
            ...
            if (added) { setFresh(new Set([added])); setTimeout(() => setFresh(new Set()), 2000); }
            return updated;
          });
        }
      ).subscribe();
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '26px auto 0', padding: '28px 30px', borderRadius: 18, background: C.surface, border: `1px solid ${C.border}` }}>
      <div style={{ ...mono, fontSize: sz(13), fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase', color: C.primary, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 12 }}>
        <span style={{ display: 'inline-block', width: 22, height: 1, background: C.primary }} />
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px 16px', alignItems: 'baseline', minHeight: sz(60) }}>
        {sorted.map(([loc, freq], i) => {
          ...
          return (
            <span key={loc} style={{ ...sans, fontSize: size, fontWeight: ratio > 0.6 ? 700 : ratio > 0.3 ? 600 : 400, color: brandColors[i % brandColors.length], opacity: 0.45 + ratio * 0.55, transition: 'all 0.5s ease', animation: isNew ? 'locFadeIn 0.6s ease' : undefined, lineHeight: 1.4 }}>
              {loc}
            </span>
          );
        })}
...
export function CommunityPulse({ C, mono, sans, serif, scale = 1 }: SlideProps) {
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '36px auto 0' }}>
      {/* Hero stat */}
      <div style={{ background: C.text, color: onDark, borderRadius: sz(24), padding: `${sz(40)}px ${sz(36)}px`, marginBottom: sz(28), position: 'relative', overflow: 'hidden', boxShadow: `0 28px 56px -16px ${C.text}40`, opacity: revealed >= 1 ? 1 : 0, transform: revealed >= 1 ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.8s cubic-bezier(0.16, 1, 0.3, 1)' }}>
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ ...mono, fontSize: sz(11), fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase' as const, color: C.primary, marginBottom: sz(12) }}>Your community ~ right now</div>
            <div style={{ ...serif, fontSize: sz(80), fontWeight: 300, letterSpacing: '-0.03em', lineHeight: 1, color: onDark }}>{animCount}</div>
            <div style={{ ...mono, fontSize: sz(12), letterSpacing: '0.14em', textTransform: 'uppercase' as const, color: 'rgba(250,248,245,0.5)', marginTop: sz(8) }}>members across all platforms</div>
          </div>
          <svg width={sz(140)} height={sz(140)} style={{ transform: 'rotate(-90deg)' }}>
            {(() => {
              ...
              return segs.map((s, i) => {
                ...
                const rot = cum * 360;
                cum += pct;
                return <circle key={i} cx={cx} cy={cy} r={r} fill="none" stroke={s.color} strokeWidth={sw} strokeDasharray={`${circ}`} strokeDashoffset={offset} strokeLinecap="round" style={{ transform: `rotate(${rot}deg)`, transformOrigin: '50% 50%', transition: 'all 1s cubic-bezier(0.16, 1, 0.3, 1)' }} />;
              });
            })()}
          </svg>
        </div>
        <div style={{ position: 'relative', display: 'flex', gap: sz(12), marginTop: sz(24) }}>
          {[
            ...
            <div key={p.label} style={{ display: 'flex', alignItems: 'center', gap: sz(8), padding: `${sz(8)}px ${sz(14)}px`, borderRadius: 100, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)' }}>
      ...
      {/* Pain points */}
      <div style={{ background: C.surface, borderRadius: sz(20), padding: `${sz(28)}px ${sz(30)}px`, border: `1px solid ${C.border}`, marginBottom: sz(28), opacity: revealed >= 2 ? 1 : 0, transform: revealed >= 2 ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.8s cubic-bezier(0.16, 1, 0.3, 1)' }}>
        ...
        <div style={{ ...serif, fontSize: sz(18), color: C.muted, marginBottom: sz(22) }}>Self-reported pain points from Skool onboarding</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: sz(10) }}>
          {painEntries.map(([key, count], i) => {
            ...
            return (
              <div key={key} onClick={() => setActivePain(isActive ? null : i)} style={{ cursor: 'pointer', transition: 'all 0.2s' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: sz(4) }}>
                  ...
                  <span style={{ ...mono, fontSize: sz(12), color: C.muted, letterSpacing: '0.08em' }}>{count} <span style={{ opacity: 0.5 }}>({((count / painTotal) * 100).toFixed(0)}%)</span></span>
                </div>
                <div style={{ height: sz(8), borderRadius: sz(4), background: `${C.surface2}`, overflow: 'hidden', position: 'relative' }}>
                  <div style={{ height: '100%', borderRadius: sz(4), background: painColors[i] || C.primary, width: revealed >= 2 ? `${pct}%` : '0%', transition: `width 1.2s cubic-bezier(0.16, 1, 0.3, 1) ${i * 0.1}s` }} />
                </div>
                {isActive && <div style={{ ...serif, fontSize: sz(13), color: C.muted, marginTop: sz(6), paddingLeft: sz(4) }}>{count} members need help with {key.toLowerCase()} ~ prime mentoring opportunity</div>}
              </div>
            );
          })}
        </div>
      ...
      {/* AI experience */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: sz(20), opacity: revealed >= 3 ? 1 : 0, transform: revealed >= 3 ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.8s cubic-bezier(0.16, 1, 0.3, 1)' }}>
        <div style={{ background: C.surface, borderRadius: sz(20), padding: `${sz(28)}px ${sz(30)}px`, border: `1px solid ${C.border}` }}>
          <div style={{ ...mono, fontSize: sz(11), fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase' as const, color: C.primary, marginBottom: sz(6), display: 'flex', alignItems: 'center', gap: sz(10) }}>
            <span style={{ display: 'inline-block', width: sz(22), height: 1, background: C.primary }} />
            AI experience levels
          </div>
          <div style={{ ...serif, fontSize: sz(16), color: C.muted, marginBottom: sz(20) }}>Where they are on their AI journey</div>
            ...
            {aiEntries.map(([level, count], i) => {
              ...
              return (
                <div key={level} style={{ display: 'flex', alignItems: 'center', gap: sz(12) }}>
                  <div style={{ width: sz(100), ...sans, fontSize: sz(13), fontWeight: 500, color: C.text, flexShrink: 0 }}>{level}</div>
                  <div style={{ flex: 1, height: sz(6), borderRadius: sz(3), background: C.surface2, overflow: 'hidden' }}>
                    <div style={{ height: '100%', borderRadius: sz(3), background: C.primary, width: revealed >= 3 ? `${pct}%` : '0%', transition: `width 1s cubic-bezier(0.16, 1, 0.3, 1) ${i * 0.12}s` }} />
                  </div>
                  <div style={{ ...mono, fontSize: sz(12), color: C.muted, width: sz(40), textAlign: 'right' as const }}>{count}</div>
                </div>
              );
            })}
          </div>
        ...
        <div style={{ background: C.text, color: onDark, borderRadius: sz(20), padding: `${sz(28)}px ${sz(30)}px`, position: 'relative', overflow: 'hidden', boxShadow: `0 16px 32px -10px ${C.text}30` }}>
          <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', backgroundImage: 'repeating-linear-gradient(0deg, rgba(255,255,255,0.03) 0px, rgba(255,255,255,0.03) 1px, transparent 1px, transparent 48px), repeating-linear-gradient(90deg, rgba(255,255,255,0.03) 0px, rgba(255,255,255,0.03) 1px, transparent 1px, transparent 48px)' }} />
          <div style={{ position: 'relative' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: sz(20) }}>
              <div>
                <div style={{ ...serif, fontSize: sz(42), fontWeight: 300, color: C.primary, lineHeight: 1 }}>{((stats.ghlOnly / (stats.ghlOnly + stats.both)) * 100).toFixed(0)}%</div>
                <div style={{ ...serif, fontSize: sz(16), color: 'rgba(250,248,245,0.7)', marginTop: sz(6) }}>of your GHL leads haven&apos;t joined Skool yet</div>
              </div>
              <div style={{ width: '100%', height: 1, background: 'rgba(255,255,255,0.08)' }} />
              <div>
                <div style={{ ...serif, fontSize: sz(42), fontWeight: 300, color: C.primary, lineHeight: 1 }}>{painEntries[0]?.[0]}</div>
                <div style={{ ...serif, fontSize: sz(16), color: 'rgba(250,248,245,0.7)', marginTop: sz(6) }}>is the #1 pain point ~ {painEntries[0]?.[1]} members need help here</div>
              </div>
              <div style={{ width: '100%', height: 1, background: 'rgba(255,255,255,0.08)' }} />
              <div>
                <div style={{ ...serif, fontSize: sz(42), fontWeight: 300, color: C.primary, lineHeight: 1 }}>{aiEntries[0]?.[0]}</div>
                <div style={{ ...serif, fontSize: sz(16), color: 'rgba(250,248,245,0.7)', marginTop: sz(6) }}>is the most common AI level ~ {aiEntries[0]?.[1]} members</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Mentor track */}
      <div style={{ background: C.surface, borderRadius: sz(20), padding: `${sz(28)}px ${sz(30)}px`, border: `1px solid ${C.border}`, marginTop: sz(28), opacity: revealed >= 4 ? 1 : 0, transform: revealed >= 4 ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.8s cubic-bezier(0.16, 1, 0.3, 1)' }}>
        <div style={{ ...mono, fontSize: sz(11), fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase' as const, color: C.primary, marginBottom: sz(18), display: 'flex', alignItems: 'center', gap: sz(10) }}>
          <span style={{ display: 'inline-block', width: sz(22), height: 1, background: C.primary }} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: sz(14) }}>
          {[
            ...
            <div key={t.letter} style={{ padding: `${sz(20)}px ${sz(18)}px`, borderRadius: sz(14), background: t.color, border: `1px solid ${C.border}` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: sz(8), marginBottom: sz(10) }}>
                <span style={{ width: sz(24), height: sz(24), borderRadius: sz(6), background: C.primary, color: onDark, display: 'flex', alignItems: 'center', justifyContent: 'center', ...mono, fontSize: sz(11), fontWeight: 800 }}>{t.letter}</span>
